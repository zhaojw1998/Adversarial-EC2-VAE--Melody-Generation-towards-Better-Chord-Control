...
def train_vae(model, train_dataloader, epoch, loss_function_vae, loss_function_discr, optimizer, writer, args, scheduler, beta=0.1):
...
def train_discr(model, train_dataloader, epoch, loss_function_vae, loss_function_discr, optimizer, writer, args, scheduler, beta=0.1):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
    losses_recon = AverageMeter()
...
def main():
    ...
    train_dataset_1 = MusicArrayLoader(train_datalist, 0.00025)
    train_dataset_2 = MusicArrayLoader(train_datalist, 0.00005)
    val_dataset = MusicArrayLoader(val_datalist, 0.0005)
    for epoch in range(init_epoch, args['n_epochs']):
        ...
        train_dataloader_2 = DataLoader(train_dataset_2, batch_size = batch_size*gpu_num,
                                    shuffle = False, drop_last = False)
        val_dataloader = DataLoader(val_dataset, batch_size = batch_size*gpu_num,
                                shuffle = False, drop_last = False)
        
        train_vae(model, train_dataloader_1, epoch, loss_function_vae, loss_function_discr, optimizer_vae, writer, args, scheduler_vae, beta=0.1)
        if args['decay'] > 0:
            scheduler_vae.step()
        train_discr(model, train_dataloader_2, epoch, loss_function_vae, loss_function_discr, optimizer_discr, writer, args, scheduler_discr, beta=0.1)
        if args['decay'] > 0:
        ...
        if epoch % 100 == 0:
            ...
            if val_loss < val_loss_record:
                ...
                torch.save({'epoch': epoch, 'model_state_dict': model.cpu().state_dict(), 'optimizer_state_dict': optimizer_full.state_dict()}, checkpoint)
                model.cuda()
